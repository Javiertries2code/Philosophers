...
{
    settings->num_ph = ft_atol(argv[1]);
    ...
    settings->philo_status = (int *)ft_calloc(settings->num_ph, sizeof(int));
    settings->return_status = ft_calloc(settings->num_ph, sizeof(int));
    settings->printer = 0;
    settings->funeral = 0;
    settings->starting_time = 0;
    settings->all_full = settings->num_ph;
  }
