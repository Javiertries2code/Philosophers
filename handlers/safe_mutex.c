...
void init_error( int error_return)
{
    ...
    if(error_return == EAGAIN)
        exit_on_error("The system lacked the necessary resources (other than memory) to initialize another mutex.");
    if(error_return == EPERM)
        exit_on_error("The caller does not have the privilege to perform the operation.");
}
void lock_error( int error_return)
{
    ...
    if(error_return == ENOTRECOVERABLE)
        exit_on_error("The mutex you are trying to\
         acquire was protecting state left irrecoverable by the mutexs previous owner.");
    if(error_return == ENOMEM)
        exit_on_error("The limit on the number of simultaneously held mutexes has been exceeded.");
}
...
void destroy_error( int error_return)
{
    ...
    if(error_return == EBUSY)
        exit_on_error("Attempt to destroy the object referenced by mutex while it was locked");
